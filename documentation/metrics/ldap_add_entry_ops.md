...
| Field | Value |
|---|---|
| **Metric Key** | `custom.freeipa.idm.ldap.add_entry_ops` |
...
Cumulative count of LDAP add (create entry) operations since the 389 Directory Server process started. In a FreeIPA environment, these correspond to new user, host, service, or policy object creation.
...
The value is read from the `addEntryOps` operational attribute.
